...
from data_scripts import BatchMolGraph, get_atom_fdim, get_bond_fdim, mol2graph
...
class MPNEncoder(nn.Module):
    ...
    def __init__(self, args: TrainArgs, atom_fdim: int, bond_fdim: int):
        ...
        if self.aggregation == 'attention':
            self.W_a = nn.Linear(self.atom_fdim + self.hidden_size, 1)
            self.att_activation = nn.Tanh()
    ...
    def forward(self,
        ...
        for depth in range(self.depth - 1):
            if self.undirected:
                raise ValueError("Should use direct passing algrithm")
        ...
        if self.aggregation == 'attention':
            attention = self.att_activation(self.W_a(a_input))
        ...
        for i, (a_start, a_size) in enumerate(a_scope):
            if a_size == 0:
                ...
                if self.aggregation == 'mean':
                    ...
                    cur_atten = attention.narrow(0, a_start, a_size)
                    cur_hiddens = cur_atten * cur_hiddens
...
class MPN(nn.Module):
    ...
    def forward(self,
        ...
        if self.use_input_features:
            ...
            features_batch = [features_batch[:, i] for i in range(features_batch.shape[1])]
        ...
        if self.use_input_features:
            # for enc, feat in zip(encodings, features_batch):
            encodings = [torch.cat([enc, feat], dim=1) for enc, feat in zip(encodings, features_batch)]
